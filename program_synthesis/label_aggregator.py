...
Label Aggregator, a generative model, copied from Snuba.

If we have time, we will try implement our own 
...
def transform_sample_stats(Xt, t, f, Xt_abs=None):
  ...
  """
  if Xt_abs is None:
    Xt_abs = abs(Xt.todense()) if sparse.issparse(Xt) else abs(Xt)
